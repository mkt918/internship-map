...
"""
梶田正規化.csv 生成スクリプト
梶田編集版.csv の F列（インターン企業名）を配置決定として採用し、
...
with open(INPUT, encoding='utf-8-sig') as f:
    ...
    for row in reader:
        ...
        eki      = canon_eki((row.get('最寄り駅') or '').strip())
...
print(f'梶田正規化.csv 出力: {len(out_rows)}件 / 未マッチ企業: {warn_count}件')
